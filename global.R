...
glmnet_initial <- c("naomit","YeoJohnson", "dummy") # <-- These are arbitrary starting values. Set these to your best recommendation
pls_initial <- c("naomit","YeoJohnson","dow", "dummy")   # <-- These are arbitrary starting values. Set these to your best recommendation
rpart_initial <- c("dow", "month") # <-- These are arbitrary starting values. Set these to your best recommendation
rf_initial <- c("impute_knn","YeoJohnson")
gbm_initial <- c("impute_median", "centre", "scale", "dummy","nzv","corr")
elm_initial<-c("impute_knn","dow", "month")
glmboost_initial <-c("naomit","dummy", "impute_knn")
nnls_initial <-c("impute_knn","dow", "dummy")
svmRadial_initial <-c("impute_knn","dow", "dummy", "corr")
rlm_initial <- c("naomit", "dow", "dummy","corr")
neuralnet_initial <- c("impute_knn","dow", "dummy") 
monmlp_initial<- c("impute_bag","dow","dummy","corr")
rvmRadial_initial <- c("impute_knn","dow", "dummy") 
svmRadialCost_initial <- c("impute_knn","dow", "dummy","corr") 
knn_initial <- c("impute_knn","dow", "dummy")
gaussprRadial_initial <- c("impute_knn","dow", "dummy","corr")
qrnn_initial <- c("impute_knn","dow", "dummy")
M5_initial <- c("impute_knn","dow", "month")
bstTree_initial<- c("impute_knn","YeoJohnson","dow", "dummy","corr")
M5Rules_initial<-c("impute_knn","dow", "month")
gaussprPoly_initial<- c("naomit", "month", "dummy")
treebag_initial <- c("impute_knn","dow", "dummy")
brnn_initial<- c("impute_knn","dow", "dummy") 
krlsPoly_initial<- c("impute_knn","dow", "dummy")
mlp_initial<- c("impute_knn","dow", "dummy") 
mlpSGD_initial<- c("impute_knn","dow", "dummy")
svmPoly_initial <- c("impute_knn","dow", "dummy")
lmStepAIC_initial<-c("naomit", "YeoJohnson", "dummy")
mxnet_initial <- c("impute_knn","dow", "dummy") 
leapSeq_initial <- c("naomit","YeoJohnson", "month", "dummy")
